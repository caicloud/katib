...
function usage {
  ...
  -n | --namespace <namespace>   The namespace where the katib-controller is installed.
...
# Set the namespace to "katib-controller" by default if not provided.
# Set the webhook service name to "katib-controller" by default if not provided.
IN_POD=false
SERVICE="katib-controller"
NAMESPACE="kube-system"
parse_arguments "$@"

TMP_DIR="/tmp/katib-controller-webhook-certs"

echo "Generating certs for the katib controller pod admission webhook in ${TMP_DIR}."
mkdir -p ${TMP_DIR}
...
if [[ "$IN_POD" == "true" ]];  then
  ...
  kubectl create secret --namespace=${NAMESPACE} generic katib-controller --from-file=${TMP_DIR}/ca-key.pem --from-file=${TMP_DIR}/ca-cert.pem --from-file=${TMP_DIR}/server-key.pem --from-file=${TMP_DIR}/server-cert.pem
fi
